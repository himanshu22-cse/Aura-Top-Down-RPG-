	...
	const FGameplayTag& InDamageType)
{
	if (FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		const TSharedPtr<FGameplayTag> DamageType = MakeShared<FGameplayTag>(InDamageType); // this will make a new shared pointer
	...
	const FVector& SphereOrigin)
{
	...
	if (const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		{
			// Implements is a template to for interface
			if (Overlap.GetActor()->Implements<UCombatInterface>() && !ICombatInterface::Execute_IsDead(Overlap.GetActor())) // As other actor whose does not undergo interface also add that's why we check which actor undergo interface
			{
				OutOverlappingActors.AddUnique(ICombatInterface::Execute_GetAvatar(Overlap.GetActor())); //AddUnique so it get any duplications
			}
		}
	}
}
...
// MakeEffectContext()->This method helps to prepare the FGameplayEffectContextHandle with any necessary data before the effect is applied, ensuring that the effect behaves correctly based on the situation in which it is used.
...
//MakeOutgoingSpec()->function is used to create and initialize a FGameplayEffectSpecHandle, which represents the specifications of a gameplay effect that will be applied to a target. This includes details like the effect's level, modifiers, and duration.
...
TArray<FRotator> UAuraAbilitySystemLibrary::EvenlySpacedRotators(const FVector& Forward, const FVector& Axis, float Spread, int32 NumRotators)
{
	...
	if (NumRotators > 1)
	{
		//We need to adjust the spread calculation by subtracting one from the NumRotators to account for the way we're launching them and ensure the desired spread.
	...
	{
		// Single Projectile (if NumRotators have only 1)
...
TArray<FVector> UAuraAbilitySystemLibrary::EvenlyRotatedVectors(const FVector& Forward, const FVector& Axis, float Spread, int32 NumVectors)
{
	...
	if (NumVectors > 1)
	{
		//We need to adjust the spread calculation by subtracting one from the NumVectors to account for the way we're launching them and ensure the desired spread.
	...
	{
		// Single Projectile(if NumVectors have only 1)
		Vectors.Add(Forward);
	}
	return Vectors;
}
