	...
	const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -.5f) // if not found tag we assign return -1 but does not means it return exact -1 it can  return close to -1 too
		{
...
// ApplyRadialDamageWithFalloff() -> apply damage to actors within a specific radius with the damage amount decreasing (falling off) as the distance from the explosion or source point increases.
	...
	const float ArmorPenetrationCoefficient = ArmorPenetrationCurve->Eval(SourcePlayerLevel); // Evaluate the curve
