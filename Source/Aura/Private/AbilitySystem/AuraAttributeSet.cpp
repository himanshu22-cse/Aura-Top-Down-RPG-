...
void UAuraAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION_NOTIFY(UAuraAttributeSet, Strength, COND_None, REPNOTIFY_Always); // COND_None->condition is none
...
//EvaluatedData: This is a member of the Data struct, likely another struct that contains details about the attribute change, such as which attribute is being changed and the new value.
...
void UAuraAttributeSet::Debuff(const FEffectProperties& Props)
{
	...
	if (DebuffTag.MatchesTagExact(GameplayTags.Debuff_Stun)) // These tags(InheritableOwnedTagsContainer) can be inherited by actors or components that receive the effect.
	{
...
void UAuraAttributeSet::HandleIncomingXP(const FEffectProperties& Props)
{
	...
	if (Props.SourceCharacter->Implements<UPlayerInterface>() && Props.SourceCharacter->Implements<UCombatInterface>())
	{
		...
		const int32 NumLevelUps = NewLevel - CurrentLevel; // Number of times we level up
		if (NumLevelUps > 0)
		{
			...
			IPlayerInterface::Execute_LevelUp(Props.SourceCharacter);  // And we call the function prefix use with 'I'
		}
